...
Alpha = 0 is handled separately — no hook is registered, just a plain forward pass. This is the baseline: flip rate should be near 0 if the probe classifies the recording correctly.

For alpha > 0, `NUM_NOISE_TRIALS` forward passes are run per `(fold, layer, alpha)` combination, each with a different pre-drawn random direction. Flip rates are averaged across trials before being stored, so `results[layer][alpha]` accumulates one value per fold (not one per trial).
...
Linear interpolation between the two alpha values that bracket the 50% flip rate threshold. Returns `inf` if 50% is never reached within the sweep range. Identical to `contrastive_patch_species.py`.
...
1. **`noise_direction_alpha_sweep.png`** — flip rate vs. alpha per layer (left) + alpha_50 bar chart (right). The bar chart title notes "compare to species direction" to prompt the direct comparison.
2. **`noise_direction_summary.png`** — heatmap of flip rate across all `(layer, alpha)` combinations, with per-cell annotations.

The gold bar marks the layer with the lowest alpha_50 under noise — this may differ from the layer identified by species-direction patching.
